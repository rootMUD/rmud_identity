  ...
  def parse_tx(tx_id) do
    ...
    IO.puts String.downcase(Map.get(event_decoded.params, "to"))
      with true <- "Transfer(address,address,uint256)" == event_decoded.event, 
           true <- String.downcase(@receipt_money_addr) == String.downcase(Map.get(event_decoded.params, "to")),
           true <- not Decimal.lt?(Decimal.new("#{Map.get(event_decoded.params, "value")}"), @price) do
