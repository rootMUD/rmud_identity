defmodule RmudIdentityWeb.CoreComponents do
  @moduledoc """
  ...
  import RmudIdentityWeb.Gettext
  ...
  def color_scheme_switch_js(assigns) do
    ...
    <script>
      ...
      window.initScheme = function() {
        if (localStorage.scheme === 'light' || (!('scheme' in localStorage) && window.matchMedia('(prefers-color-scheme: dark)').matches)) {
          applyScheme("light")
        } else {
          applyScheme("dark")
        }
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(RmudIdentityWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(RmudIdentityWeb.Gettext, "errors", msg, opts)
    end
  end
